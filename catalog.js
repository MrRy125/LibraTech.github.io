...
{ id: 135, title: 'Magento', author: 'Tutorialspoint', category: 'Technology Topics', downloadLink: '/books/TechTopics/Magento.pdf', imageUrl: 'images/book170.png' },
...
// Function to display books based on filter/search and pagination
function displayBooks(pageNumber) {
    ...
    const filteredBooks = books.filter((book) => {
      ...
      return category === 'all' ? isMatch : isMatch && book.category === category;
    });
  
    const booksPerPage = 32; // Number of books to display per page
    ...
    booksToDisplay.forEach((book) => {
      ...
      bookCard.innerHTML = `
        ...
        <button class="details-btn" data-id="${book.id}">Book Details</button>
        <button class="download-btn" data-download="${book.downloadLink}">Download</button>
      `;
      bookListContainer.appendChild(bookCard);
    });
  
    // Add event listeners to details and download buttons
    document.querySelectorAll('.details-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        const bookId = btn.getAttribute('data-id');
        // Redirect to Book Details page with specific book ID
  ...
  document.getElementById('search').addEventListener('input', function () {
    displayBooks(1); // Reset to the first page when search input changes
  });
  document.getElementById('category').addEventListener('change', function () {
    displayBooks(1); // Reset to the first page when category filter changes
  });
  
  // Initial display of books on the first page
  displayBooks(1);
